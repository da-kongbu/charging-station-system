...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT COUNT(o) FROM Order o WHERE o.status IN (2, 3) AND o.createdAt BETWEEN :start AND :end")
    Long countCompletedOrdersBetween(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    @Query("SELECT SUM(o.totalAmount) FROM Order o WHERE o.status IN (2, 3) AND o.createdAt BETWEEN :start AND :end")
